...
def evaluate(generator, model, threshold=0.01, debug=False):
    ...
    results = []
    image_ids = []
    for index in trange(generator.size(), desc='FLIR evaluation: '):
...
if __name__ == '__main__':
    ...
    model_path = '../checkpoints/flir_50_0.1703_0.3104.h5'
    common_args = {
        'batch_size': 1,
        'phi': phi,
    }

    test_generator = FlirGenerator(
        'G:\datasets\FLIR',
        'mytest',
    ...
    evaluate(test_generator, prediction_model, threshold=0.1, debug=True)
